...
export class PokemonDetails implements AfterViewInit, OnDestroy {
  @ViewChild('detailContainer', { static: true })
  detailContainer!: ElementRef<HTMLElement>;
  ...
  getIta(obj: any) {
    const entry = obj.flavor_text_entries?.find(
  ...
  ngAfterViewInit() {
    ...
    // osserva resize
    this.stopObserver = this.size.observeSize(el, () => updateHeight());
  }
